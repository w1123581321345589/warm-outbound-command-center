...
export class DatabaseStorage implements IStorage {
  ...
  async getProspects(teamId: number, filters?: { stage?: string, assignedToId?: string }): Promise<Prospect[]> {
    const conditions = [eq(prospects.teamId, teamId)];

    if (filters?.stage) conditions.push(eq(prospects.stage, filters.stage));
    if (filters?.assignedToId) conditions.push(eq(prospects.assignedToId, filters.assignedToId));
  ...
  async getQCItems(filters?: { status?: string }): Promise<QCQueueItem[]> {
    ...
    if (filters?.status) conditions.push(eq(qcQueueItems.status, filters.status));
  ...
  async createProspectsBulk(prospectData: InsertProspect[]): Promise<Prospect[]> {
    ...
    return await db.insert(prospects).values(prospectData).returning();
  }

  async getAnalyticsOverview(): Promise<any> {
    const [{ count: totalProspects }] = await db.select({ count: sql<number>`count(*)` }).from(prospects);
    const [{ count: pendingTasks }] = await db.select({ count: sql<number>`count(*)` }).from(tasks).where(eq(tasks.status, TaskStatus.PENDING));
    const [{ count: pendingQC }] = await db.select({ count: sql<number>`count(*)` }).from(qcQueueItems).where(eq(qcQueueItems.status, QCStatus.PENDING));
    ...
    return {
      prospectsByStage,
      tasksDueToday: Number(pendingTasks),
      qcPending: Number(pendingQC),
      replyRate: 12.5
    };
  }
}
